...
uint64_t mult8_cgp14ep_ep64716_wc7_3_csamrca(const uint64_t B,const uint64_t A)
{
   ...
   dout_63=dout_60|dout_35;
   dout_64=dout_38^dout_18;
   dout_65=dout_38&dout_18;
   ...
   dout_95=((B >> 6)&1)&((A >> 3)&1);
   dout_96=((B >> 7)&1)&((A >> 3)&1);
   dout_97=dout_62^dout_55;
   dout_98=dout_62&dout_55;
...
uint32_t mul16u_pwr_2_181_mre_00_0000 (uint16_t a, uint16_t b) {
    ...
    if(fillData) {
        for(int i = 0; i < 256; i++) {
            for(int j = 0; j < 256; j++) {
                cacheLL[i * 256 + j] = mult8_cgp14ep_ep64716_wc7_3_csamrca(i, j);
