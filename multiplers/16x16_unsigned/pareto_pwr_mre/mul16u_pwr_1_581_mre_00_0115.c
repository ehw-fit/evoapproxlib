...
uint64_t mult8_cgp14ep_ep64716_wc40_csamrca(const uint64_t B,const uint64_t A)
{
...
uint32_t mul16u_pwr_1_581_mre_00_0115 (uint16_t a, uint16_t b) {
    ...
    if(fillData) {
        for(int i = 0; i < 256; i++) {
            for(int j = 0; j < 256; j++) {
                ...
                cacheLH[i * 256 + j] = mult8_cgp14ep_ep64716_wc40_csamrca(i, j);
                cacheHL[i * 256 + j] = mult8_cgp14ep_ep64716_wc40_csamrca(i, j);
