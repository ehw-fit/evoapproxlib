    ...
    uint64_t mult8_cgp14_wc2837_rcam(const uint64_t B,const uint64_t A)
{
   ...
   dout_323=dout_321&dout_317;
   dout_324=dout_321^dout_317;
...
uint32_t mul16u_pwr_0_084_mae_01_2640 (uint16_t a, uint16_t b) {
    ...
    if(fillData) {
        for(int i = 0; i < 256; i++) {
            for(int j = 0; j < 256; j++) {
                ...
                cacheHH[i * 256 + j] = mult8_cgp14_wc2837_rcam(i, j);
            }
        }
    }
...
// internal reference: composition.16.m16_m3_composed_wc_pwr_ar_4215
